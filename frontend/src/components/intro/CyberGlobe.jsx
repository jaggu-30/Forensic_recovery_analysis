...
function DigitalSurface({ texture, progress }) {
  ...
  useFrame((state) => {
    ...
    material.uniforms.uOpacity.value = THREE.MathUtils.lerp(1.0, 0.16, THREE.MathUtils.smoothstep(progress, 0.38, 0.76));
  });
...
function Grid({ progress }) {
  const opacity = THREE.MathUtils.lerp(0.10, 0.012, progress);
  const lines = [];
    ...
    <group ref={ref}>
      <Points positions={points.flatMap((p) => p.toArray())} stride={3}>
        <PointMaterial
          ...
          sizeAttenuation
          depthWrite={false}
          opacity={THREE.MathUtils.lerp(0.92, 0.16, progress)}
          blending={THREE.AdditiveBlending}
        />
      </Points>
    </group>
...
function Connections({ progress }) {
  ...
  const opacity = THREE.MathUtils.lerp(0.48, 0.06, progress);
...
function OrbitRings({ progress }) {
  ...
  const opacity = THREE.MathUtils.lerp(0.58, 0.08, progress);
  return (
...
export default function CyberGlobe({ progress }) {
  ...
  useFrame((state, delta) => {
    ...
    const scale = THREE.MathUtils.lerp(1.08, 0.94, THREE.MathUtils.smoothstep(progress, 0.42, 0.82));
    group.current.scale.setScalar(scale);
    group.current.position.y = THREE.MathUtils.lerp(0.12, 0.48, progress);
  });
